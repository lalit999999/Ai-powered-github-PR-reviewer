...
- **`email` carries `@unique`, which phase-01 §6 does not specify.** The adapter's
  ...
  _stricter_ than the document, not looser. `email` stays nullable — GitHub accounts
...
`session` callback does this conversion, so `session.user.githubUserId` is _always_ a
...
`req.baseUrl` to the _entire_ request path. Measured directly:
...
**A plain prefix mount is what Express 5 actually needs here.** `basePath` is _also_ set
...
## 10. Inngest tracing middleware needs _two_ hooks, not one
...
| Test                                     | Change                                                                            | Why                                                                                                                                                                                  |
| ---------------------------------------- | --------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `db.test.ts` "no premature schema creep" | Rewritten against the field-complete column list                                  | Phase 01 §6 supersedes the placeholder shape _by design_; the assertion was correct for Phase 00 and is now correct for Phase 01. Kept (not deleted) so schema creep is still caught |
...
phase-00-log §9 deferred `packages/shared` to Phase 03 — but that was about _sharing a
duplicated logger_, which is convenience. A cross-deployable contract is a different
...
`GET`/`DELETE /api/projects/:id`. §12 says a 403 that reveals a resource _exists_ but
...
Ownership is unaffected: a _foreign_ soft-deleted project is still 404, which is asserted
...
"doesn't exist" indistinguishable _in the logs_, and §20 requires the warn line to
...
`findSlugsForUserByPrefix` is the other named exception — to the _exclude-deleted_ rule,
...
with enough contention the single retry _should_ lose, and 409 is the correct answer
...
carrying a _forged or expired_ cookie passes layer 1 and is rejected by layer 2 — but the
...
Making the Edge layer _validate_ (rather than filter) would produce a true 307, at the
...
The list query **is** index-served at any realistic tenant size — just by the _unique_
...
still carries `userId`, so only the _offset_ is affected, never the contents.
