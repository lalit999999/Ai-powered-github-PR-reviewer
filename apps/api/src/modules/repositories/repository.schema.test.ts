...
describe("parseGithubRepoUrl — accepted forms (phase-02 §13)", () => {
  it("accepts the canonical form and returns the parsed pair, not a boolean", () => {
    expect(
      parseGithubRepoUrl("https://github.com/octocat/hello-world"),
    ).toEqual({
      ok: true,
      owner: "octocat",
      repo: "hello-world",
  ...
  it.each([
    ...
    [
      ...
      "https's default port is not an explicit port",
    ],
...
describe("parseGithubRepoUrl — rejections (the SSRF table)", () => {
  const cases: Array<[string, GithubRepoUrlRejection, string]> = [
    [
      "http://github.com/o/r",
      "BAD_SCHEME",
      "a TLS downgrade is never accepted",
    ],
    ["ftp://github.com/o/r", "BAD_SCHEME", "no other scheme is either"],
    [
      "javascript:alert(1)//github.com/o/r",
      "BAD_SCHEME",
      "a script URL that mentions the host",
    ],
    [
      "https://github.com.evil.com/o/r",
      "HOST_NOT_ALLOWED",
      "attacker host CONTAINING the allowed one",
    ],
    [
      "https://evil.com/github.com/o/r",
      "HOST_NOT_ALLOWED",
      "allowed host appearing in the path",
    ],
    [
      "https://githubXcom/o/r",
      "HOST_NOT_ALLOWED",
      "an unescaped dot in a naive regex matches this",
    ],
    [
      "https://user:pass@github.com/o/r",
      "USERINFO_PRESENT",
      "the classic parser-confusion payload",
    ],
    [
      "https://github.com@evil.com/o/r",
      "USERINFO_PRESENT",
      "allowed host as the userinfo of another",
    ],
    ["https://github.com:8080/o/r", "PORT_NOT_ALLOWED", "a non-default port"],
    [
      "https://github.com/o",
      "BAD_PATH",
      "one path segment is not a repository",
    ],
    ["https://github.com/", "BAD_PATH", "no path segments at all"],
    [
      "https://github.com/o/r/tree/main",
      "BAD_PATH",
      "extra segments beyond owner/repo",
    ],
    ...
    [
      ...
      "BAD_PATH",
      "an owner may not start with a hyphen",
    ],
    [
      "//github.com/o/r",
      "NOT_A_URL",
      "scheme-relative — not a URL without a base",
    ],
  ...
  it("rejects an over-long string even when it is not a URL at all", () => {
    expect(parseGithubRepoUrl("x".repeat(10_000))).toEqual({
      ok: false,
      reason: "TOO_LONG",
    });
  });
...
describe("githubRepoUrlSchema — the Zod face of the same parser", () => {
  it("transforms an accepted URL into the parsed pair", () => {
    expect(
      githubRepoUrlSchema.parse("https://github.com/octocat/hello-world.git"),
    ).toEqual({
      owner: "octocat",
      repo: "hello-world",
    });
...
describe("connectRepositoryBodySchema — exactly one of repoUrl / githubRepoId (phase-02 §7)", () => {
  it("accepts a URL alone", () => {
    expect(
      connectRepositoryBodySchema.parse({
  ...
  it("accepts an id alone and coerces it to a bigint", () => {
    const parsed = connectRepositoryBodySchema.parse({
  ...
  it("rejects a bad URL with the actionable message before the service ever sees it", () => {
    const result = connectRepositoryBodySchema.safeParse({
      repoUrl: "https://github.com.evil.com/o/r",
    });
    expect(result.success).toBe(false);
    expect(result.error?.issues[0]?.message).toBe(GITHUB_REPO_URL_MESSAGE);
  });
  ...
  it("rejects a JSON number for githubRepoId rather than losing precision", () => {
    ...
    expect(
      connectRepositoryBodySchema.safeParse({ githubRepoId: 1296269 }).success,
    ).toBe(false);
    ...
    expect(
      connectRepositoryBodySchema.safeParse({
        githubRepoId: Number.MAX_SAFE_INTEGER + 2,
      }).success,
    ).toBe(false);
  });
  ...
  ])("rejects %j as a githubRepoId", (value) => {
    expect(
      connectRepositoryBodySchema.safeParse({ githubRepoId: value }).success,
    ).toBe(false);
  });
...
describe("params and query schemas", () => {
  it("accepts any non-empty repositoryId, so a malformed id 404s rather than 400s", () => {
    ...
    expect(
      repositoryIdParamSchema.parse({ repositoryId: "not-a-uuid" }),
    ).toEqual({ repositoryId: "not-a-uuid" });
    expect(
      repositoryIdParamSchema.safeParse({ repositoryId: "" }).success,
    ).toBe(false);
  });

  it("coerces installationId to a bigint and rejects a non-numeric one", () => {
    expect(installationIdParamSchema.parse({ installationId: "42" })).toEqual({
      installationId: 42n,
    });
    expect(
      installationIdParamSchema.safeParse({ installationId: "abc" }).success,
    ).toBe(false);
  });

  it("treats an empty or whitespace-only ?q as no filter", () => {
    expect(listInstallationReposQuerySchema.parse({})).toEqual({
      q: undefined,
    });
    expect(listInstallationReposQuerySchema.parse({ q: "   " })).toEqual({
      q: undefined,
    });
    expect(listInstallationReposQuerySchema.parse({ q: "  api  " })).toEqual({
      q: "api",
    });
  });
  ...
  it("rejects an over-long ?q", () => {
    expect(
      listInstallationReposQuerySchema.safeParse({ q: "a".repeat(101) })
        .success,
    ).toBe(false);
  });
});
...
describe("triggerIndexBodySchema — INCREMENTAL is explicitly rejected, not merely absent (§7)", () => {
  it("accepts mode: FULL", () => {
    expect(triggerIndexBodySchema.safeParse({ mode: "FULL" }).success).toBe(
      true,
    );
  });
    ...
    if (!result.success) {
      expect(
        result.error.issues.some(
          (issue) => issue.message === INCREMENTAL_NOT_SUPPORTED_MESSAGE,
        ),
      ).toBe(true);
    }
  });
  ...
  it("rejects a mode that is neither FULL nor INCREMENTAL", () => {
    expect(triggerIndexBodySchema.safeParse({ mode: "PARTIAL" }).success).toBe(
      false,
    );
  });
