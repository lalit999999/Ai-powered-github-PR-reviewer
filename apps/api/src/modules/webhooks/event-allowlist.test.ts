...
import { isAllowedEvent, isReviewTriggeringAction, REVIEW_TRIGGERING_ACTIONS } from "./event-allowlist.js";

describe("isAllowedEvent", () => {
  it.each(["pull_request", "installation", "installation_repositories", "repository", "push", "ping"])(
    "allows %s",
    (eventType) => {
...
describe("isReviewTriggeringAction", () => {
  it.each(REVIEW_TRIGGERING_ACTIONS)("narrows %s as review-triggering", (action) => {
    expect(isReviewTriggeringAction(action)).toBe(true);
  });

  it.each(["edited", "closed", "converted_to_draft"])("does not narrow %s as review-triggering", (action) => {
