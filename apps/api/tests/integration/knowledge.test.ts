...
vi.mock("../../src/inngest/emit.js", () => ({ emitRepositoryIndexRequested: vi.fn(), emitProjectDeleted: vi.fn() }));
vi.mock("@repo/github", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@repo/github")>();
  return {
    ...actual,
    installationGithub: { listInstallationRepositories: vi.fn(), listUserInstallations: vi.fn() },
...
async function connectRepository(name: string): Promise<{ projectId: string; repositoryId: string }> {
  const projectRes = await request(app).post("/api/projects").set("Cookie", user.cookie).send({ name: `Knowledge Test ${name}` });
...
async function seedKnowledgeGraph(repositoryId: string): Promise<void> {
  ...
  const symA = await prisma.codeSymbol.create({
    data: { repositoryId, fileId: fileA.id, name: "a", kind: "FUNCTION", startLine: 1, endLine: 2, isExported: true, isDefault: false, complexity: 1, commitSha: "sha1" },
  });
  const symB = await prisma.codeSymbol.create({
    data: { repositoryId, fileId: fileA.id, name: "b", kind: "FUNCTION", startLine: 3, endLine: 4, isExported: true, isDefault: false, complexity: 1, commitSha: "sha1" },
  });

  await prisma.codeDependency.createMany({
    data: [
      { id: randomUUID(), repositoryId, kind: "CALLS", fromSymbolId: symA.id, toSymbolId: symB.id, resolution: "RESOLVED", confidence: 0.95, commitSha: "sha1" },
      { id: randomUUID(), repositoryId, kind: "IMPORTS", fromFileId: fileA.id, toFileId: fileB.id, resolution: "RESOLVED", confidence: 1, commitSha: "sha1" },
      { id: randomUUID(), repositoryId, kind: "IMPORTS", fromFileId: fileA.id, externalPackage: "zod", resolution: "EXTERNAL", confidence: 1, commitSha: "sha1" },
      { id: randomUUID(), repositoryId, kind: "IMPORTS", fromFileId: fileB.id, rawSpecifier: "./does-not-exist.js", resolution: "UNRESOLVED", confidence: 1, commitSha: "sha1" },
    ],
  });
}
...
describe("GET /api/repositories/:id/knowledge (phase-04 §7)", () => {
  it("200s with the correct aggregates for an owned, indexed repository", async () => {
    ...
    const res = await request(app).get(`/api/repositories/${repositoryId}/knowledge`).set("Cookie", user.cookie);
    ...
    expect(res.body.topFilesByInboundEdges).toEqual(
      expect.arrayContaining([expect.objectContaining({ path: "src/a.ts", inboundEdgeCount: 1 })]),
    );
    expect(res.body.topUnresolvedSpecifiers).toEqual([{ rawSpecifier: "./does-not-exist.js", count: 1 }]);
  ...
  it("200s with all-zero aggregates for a repository with no knowledge graph yet", async () => {
    ...
    expect(res.status).toBe(200);
    expect(res.body).toMatchObject({ fileCount: 0, symbolCount: 0, edgeCount: 0, unresolvedImportRatio: 0, topFilesByInboundEdges: [], topUnresolvedSpecifiers: [] });
  });
  ...
  it("404s for a repository belonging to another user", async () => {
    ...
    const res = await request(app).get(`/api/repositories/${owner.repositoryId}/knowledge`).set("Cookie", otherUser.cookie);
  ...
  it("404s for a non-existent repository id", async () => {
    const res = await request(app).get("/api/repositories/00000000-0000-0000-0000-000000000000/knowledge").set("Cookie", user.cookie);
    expect(res.status).toBe(404);
  });

  it("404s for a repository under a soft-deleted project", async () => {
    const { projectId, repositoryId } = await connectRepository("knowledge-deleted-project");

    const deleteRes = await request(app).delete(`/api/projects/${projectId}`).set("Cookie", user.cookie);
