...
async function runWithConcurrency(
  count: number,
  worker: (index: number) => Promise<number>,
): Promise<number[]> {
...
describe.skipIf(process.env.SKIP_PERF_TESTS === "1")(
  "POST /api/webhooks/github — acknowledgment latency (§14/§15)",
  () => {
    ...
    it(`acknowledges ${DELIVERY_COUNT.toString()} concurrent deliveries at p99 < 500ms`, async () => {
      ...
      const durationsMs = await runWithConcurrency(
        DELIVERY_COUNT,
        async (index) => {
          const { text } = loadWebhookFixture("pull-request-opened.json", {
            ...
            mutate: (payload) => {
              ...
              (pr.head as Record<string, unknown>).sha =
                `latency-test-sha-${index.toString().padStart(6, "0")}`;
            },
          });
