...
async function fetchKnowledge(repositoryId: string): Promise<RepositoryKnowledge> {
  const res = await fetch(`${API_URL}/api/repositories/${encodeURIComponent(repositoryId)}/knowledge`, {
    credentials: "include",
  });
  if (!res.ok) {
...
export function KnowledgePanel({ repositoryId, indexStatus }: { repositoryId: string; indexStatus: string }) {
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Symbols and dependency edges extracted from this repository&apos;s source.</CardDescription>
      </CardHeader>
      <CardContent>
        ...
        {error && (
          <p role="alert" className="text-sm text-destructive">
            Could not load the knowledge graph. {error instanceof Error ? error.message : ""}
          </p>
        )}
  ...
  if (data.symbolCount === 0) {
    return (
      <p className="text-sm text-muted-foreground">
        No knowledge graph yet — this repository was indexed before Phase 04 shipped, or its parse produced no
        symbols. Re-index to build it.
      </p>
    );
  }

  const unresolvedRatioIsHigh = data.unresolvedImportRatio > UNRESOLVED_RATIO_WARNING_THRESHOLD;
    ...
    <div className="flex flex-col gap-6">
      ...
      <div>
        ...
        <div className="mt-1 flex items-center gap-2">
          <Badge variant={unresolvedRatioIsHigh ? "destructive" : "outline"}>{formatPercent(data.unresolvedImportRatio)}</Badge>
          {unresolvedRatioIsHigh && (
            <span className="text-xs text-destructive">
              Above the 15% health threshold — usually a missed tsconfig read or an unusual bundler alias.
            </span>
          )}
        </div>
      ...
      <div>
        ...
        <div className="mt-1 flex flex-wrap gap-2">
          {Object.entries(data.parseStateCounts).map(([state, count]) => (
            <Badge key={state} variant={state === "FAILED" && count > 0 ? "destructive" : "outline"}>
              {state}: {count}
            </Badge>
          ))}
        </div>
        {failedFiles > 0 && (
          <p className="mt-1 text-xs text-muted-foreground">
            {failedFiles} file{failedFiles === 1 ? "" : "s"} failed to parse and stayed text-indexed only.
          </p>
        )}
      </div>
      ...
      <div>
        ...
        {data.topFilesByInboundEdges.length === 0 ? (
          <p className="mt-1 text-sm text-muted-foreground">No inbound edges recorded.</p>
        ) : (
            ...
            <TableBody>
              {data.topFilesByInboundEdges.map((file) => (
                <TableRow key={file.fileId}>
                  ...
                  <TableCell className="text-right">{file.inboundEdgeCount}</TableCell>
                </TableRow>
              ))}
            </TableBody>
          </Table>
