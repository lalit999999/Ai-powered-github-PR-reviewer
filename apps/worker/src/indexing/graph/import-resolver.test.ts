...
import type { PackageManifest, RepoContext, TsconfigEntry } from "./repo-context.js";

function pkg(overrides: Partial<PackageManifest> & { dir: string }): PackageManifest {
  return {
    dir: overrides.dir,
...
function tsconfig(overrides: Partial<TsconfigEntry> & { dir: string }): TsconfigEntry {
  return { dir: overrides.dir, path: `${overrides.dir}/tsconfig.json`, baseUrl: overrides.baseUrl, paths: overrides.paths };
}
...
describe("resolveImport — step 1: relative specifiers", () => {
  it("resolves a .ts sibling for an extensionless specifier", () => {
    ...
    expect(resolveImport("./foo", "src/bar.ts", ctx)).toEqual({ status: "RESOLVED", targetFilePath: "src/foo.ts" });
  });
  ...
  it("resolves a .js specifier to its .ts sibling — mandatory ESM/NodeNext interop", () => {
    const ctx = context({ files: ["src/foo.ts", "src/bar.ts"] });
    expect(resolveImport("./foo.js", "src/bar.ts", ctx)).toEqual({ status: "RESOLVED", targetFilePath: "src/foo.ts" });
  });

  it("falls back to a literal .js file when no .ts sibling exists (plain JS repo)", () => {
    const ctx = context({ files: ["src/foo.js", "src/bar.js"] });
    expect(resolveImport("./foo.js", "src/bar.js", ctx)).toEqual({ status: "RESOLVED", targetFilePath: "src/foo.js" });
  });
  ...
  it("resolves a directory import to its index file, only after direct extensions fail", () => {
    ...
    expect(resolveImport("./utils", "src/bar.ts", ctx)).toEqual({ status: "RESOLVED", targetFilePath: "src/utils/index.ts" });
  });

  it("prefers a direct file over a same-named directory's index file", () => {
    const ctx = context({ files: ["src/utils.ts", "src/utils/index.ts", "src/bar.ts"] });
    expect(resolveImport("./utils", "src/bar.ts", ctx)).toEqual({ status: "RESOLVED", targetFilePath: "src/utils.ts" });
  });

  it("resolves ../ parent-directory specifiers", () => {
    const ctx = context({ files: ["src/shared.ts", "src/nested/child.ts"] });
    expect(resolveImport("../shared", "src/nested/child.ts", ctx)).toEqual({ status: "RESOLVED", targetFilePath: "src/shared.ts" });
  });

  it("is UNRESOLVED for a genuinely missing relative target", () => {
    const ctx = context({ files: ["src/bar.ts"] });
    expect(resolveImport("./nope", "src/bar.ts", ctx)).toEqual({ status: "UNRESOLVED", specifier: "./nope" });
  });
  ...
  it("never leaves the repository root — a traversal escape resolves to nothing, not a garbage path", () => {
    ...
    const result = resolveImport("../../../../../../etc/passwd", "src/bar.ts", ctx);
    expect(result).toEqual({ status: "UNRESOLVED", specifier: "../../../../../../etc/passwd" });
  });

  it("treats an absolute-path specifier as unresolved, never as a real filesystem path", () => {
    const ctx = context({ files: ["etc/passwd.ts"] });
    expect(resolveImport("/etc/passwd", "src/bar.ts", ctx)).toEqual({ status: "UNRESOLVED", specifier: "/etc/passwd" });
  });
});
...
describe("resolveImport — step 2: tsconfig paths alias", () => {
  ...
  it("prefers the longest matching prefix among overlapping wildcard aliases", () => {
    const ctx = context({
      ...
      tsconfigs: [tsconfig({ dir: "", paths: { "@/*": ["src/*"], "@/lib/*": ["packages/shared/src/*"] } })],
    });
  ...
  it("uses the nearest ancestor tsconfig for a file under a nested package", () => {
    const ctx = context({
      ...
      tsconfigs: [
        tsconfig({ dir: "", paths: { "@shared/*": ["packages/shared/src/*"] } }),
...
describe("resolveImport — step 3: workspace package", () => {
  const workspaceContext = context({
    files: ["packages/ui/src/index.ts", "packages/ui/package.json", "apps/web/src/app.ts"],
    packages: [pkg({ dir: "packages/ui", name: "@repo/ui", main: "src/index.ts" }), pkg({ dir: "apps/web", name: "web" })],
    workspaceRoots: ["packages/ui", "apps/web"],
  });

  it("resolves a bare workspace package name to its declared main entry point", () => {
    expect(resolveImport("@repo/ui", "apps/web/src/app.ts", workspaceContext)).toEqual({
      status: "RESOLVED",
  ...
  it("resolves a workspace package subpath relative to the package root when there is no exports map", () => {
    const ctx = context({
      ...
      packages: [pkg({ dir: "packages/ui", name: "@repo/ui" }), pkg({ dir: "apps/web", name: "web" })],
      workspaceRoots: ["packages/ui", "apps/web"],
    });
    expect(resolveImport("@repo/ui/src/Button", "apps/web/src/app.ts", ctx)).toEqual({
      status: "RESOLVED",
  ...
  it("resolves via an exports map's exact subpath key", () => {
    const ctx = context({
      ...
      packages: [
        pkg({ dir: "packages/ui", name: "@repo/ui", exports: { ".": "./src/index.ts", "./button": "./src/button.ts" } }),
...
describe("resolveImport — step 4: bare specifiers (node builtins and external packages)", () => {
  it("treats a node: prefixed specifier as EXTERNAL", () => {
    ...
    expect(resolveImport("node:fs", "src/app.ts", ctx)).toEqual({ status: "EXTERNAL", packageName: "node:fs" });
  });

  it("treats a bare builtin module name as EXTERNAL", () => {
    const ctx = context({ files: ["src/app.ts"] });
    expect(resolveImport("path", "src/app.ts", ctx)).toEqual({ status: "EXTERNAL", packageName: "path" });
  });

  it("treats an ordinary bare package specifier as EXTERNAL, with its version from package.json", () => {
    const ctx = context({
      files: ["src/app.ts", "package.json"],
      packages: [pkg({ dir: "", name: "root", dependencies: { lodash: "^4.17.21" } })],
    });
    expect(resolveImport("lodash", "src/app.ts", ctx)).toEqual({ status: "EXTERNAL", packageName: "lodash", version: "^4.17.21" });
  });

  it("resolves a scoped bare package's name correctly (not the whole specifier)", () => {
    const ctx = context({ files: ["src/app.ts"] });
    expect(resolveImport("@sentry/node", "src/app.ts", ctx)).toEqual({ status: "EXTERNAL", packageName: "@sentry/node" });
  });
...
describe("resolveImport — step 5: genuinely unresolvable specifiers", () => {
  it("is UNRESOLVED for an empty specifier", () => {
    const ctx = context({ files: ["src/app.ts"] });
    expect(resolveImport("", "src/app.ts", ctx)).toEqual({ status: "UNRESOLVED", specifier: "" });
  });

  it("is UNRESOLVED for a malformed scoped specifier with no package segment", () => {
    const ctx = context({ files: ["src/app.ts"] });
    expect(resolveImport("@justscope", "src/app.ts", ctx)).toEqual({ status: "UNRESOLVED", specifier: "@justscope" });
  });
});
