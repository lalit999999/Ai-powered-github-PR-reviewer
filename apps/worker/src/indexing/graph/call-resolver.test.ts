...
function sym(
  ...
  opts: { calls?: ParsedCall[]; parentSymbol?: string } = {},
): {
  id: string;
...
function buildFile(spec: FileSpec): CallResolverFileInput {
  ...
  const importResolutions = new Map(
    Object.entries(spec.importResolutions ?? {}),
  );
  return {
...
describe("resolveCalls — rule 1: same-file", () => {
  it("resolves a call to a same-file definition over an identically-named symbol elsewhere in the repo", () => {
    ...
    const otherFile = buildFile({
      filePath: "packages/ui/src/format.ts",
      packageName: "@repo/ui",
      entries: [helperElsewhere],
    });
  ...
  it("emits a self-recursion edge (documented decision: emit, not skip)", () => {
    const factorial = sym("factorial", "FUNCTION", {
      calls: [call("factorial")],
    });
    const file = buildFile({
      filePath: "apps/web/src/lib/math.ts",
      packageName: "web",
      entries: [factorial],
    });
    ...
    expect(result.edges).toEqual([
      {
        fromSymbolId: factorial.id,
        toSymbolId: factorial.id,
        rule: "SAME_FILE",
        confidence: CALL_CONFIDENCE_SAME_FILE,
      },
    ]);
  ...
  it("resolves this.foo() to the enclosing class's own same-named method", () => {
    ...
    const method = sym("update", "METHOD", {
      calls: [call("save", "this")],
      parentSymbol: "UserRepo",
    });
    const target = sym("save", "METHOD", { parentSymbol: "UserRepo" });

    const otherFile = buildFile({
      filePath: "packages/db/src/other-repo.ts",
      packageName: "@repo/db",
      entries: [other],
    });
    const file = buildFile({
      filePath: "apps/web/src/lib/user-repo.ts",
      packageName: "web",
      entries: [method, target],
    });
...
describe("resolveCalls — rule 2: named import", () => {
  it("resolves a bare call via a named import, over an unrelated same-named symbol elsewhere", () => {
    ...
    const caller = sym("handleSubmit", "FUNCTION", {
      calls: [call("validate")],
    });

    const targetFile = buildFile({
      filePath: "packages/shared/src/validate.ts",
      packageName: "@repo/shared",
      entries: [realTarget],
    });
    const decoyFile = buildFile({
      filePath: "packages/other/src/validate.ts",
      packageName: "@repo/other",
      entries: [decoy],
    });
    const callerFile = buildFile({
      ...
      entries: [caller],
      imports: [
        namedImport("../../../../packages/shared/src/validate.js", [
          "validate",
        ]),
      ],
      importResolutions: {
        "../../../../packages/shared/src/validate.js": {
          status: "RESOLVED",
          targetFilePath: "packages/shared/src/validate.ts",
        },
      },
    });
  ...
  it("resolves a namespace-import member call to the target module's export", () => {
    ...
    const callerFile = buildFile({
      ...
      importResolutions: {
        "./lib/utils.js": {
          status: "RESOLVED",
          targetFilePath: "apps/web/src/lib/utils.ts",
        },
      },
    });
...
describe("resolveCalls — rule 3: unique repo-wide match", () => {
  it("resolves when exactly one exported symbol repo-wide matches the call name", () => {
    ...
    const caller = sym("render", "FUNCTION", {
      calls: [call("formatCurrency")],
    });

    const targetFile = buildFile({
      filePath: "packages/shared/src/money.ts",
      packageName: "@repo/shared",
      entries: [target],
    });
    const callerFile = buildFile({
      filePath: "apps/web/src/components/Price.tsx",
      packageName: "web",
      entries: [caller],
    });
  ...
  it("excludes non-callable kinds from candidates entirely", () => {
    ...
    const declFile = buildFile({
      filePath: "packages/shared/src/types.ts",
      packageName: "@repo/shared",
      entries: [iface, typeAlias],
    });
    const callerFile = buildFile({
      filePath: "apps/web/src/dispatch.ts",
      packageName: "web",
      entries: [caller],
    });
...
describe("resolveCalls — rule 4: ambiguous matches", () => {
  it("narrows to the same-package candidate when the caller shares a package with exactly one match", () => {
    ...
    const f1 = buildFile({
      filePath: "apps/web/src/lib/handler.ts",
      packageName: "web",
      entries: [samePackage],
    });
    const f2 = buildFile({
      filePath: "packages/ui/src/handler.ts",
      packageName: "@repo/ui",
      entries: [otherPackage1],
    });
    const f3 = buildFile({
      filePath: "packages/db/src/handler.ts",
      packageName: "@repo/db",
      entries: [otherPackage2],
    });
    const callerFile = buildFile({
      filePath: "apps/web/src/routes/index.ts",
      packageName: "web",
      entries: [caller],
    });
    ...
    expect(edges).toEqual([
      {
        fromSymbolId: caller.id,
        toSymbolId: samePackage.id,
        rule: "AMBIGUOUS_TIEBREAK",
        confidence: 0.4,
      },
    ]);
  });
  ...
  it("falls back to same-top-level-directory when no candidate shares the caller's package", () => {
    ...
    const f1 = buildFile({
      filePath: "apps/admin/src/handler.ts",
      packageName: "admin",
      entries: [sameTopLevel],
    });
    const f2 = buildFile({
      filePath: "packages/db/src/handler.ts",
      packageName: "@repo/db",
      entries: [otherTopLevel],
    });
    const callerFile = buildFile({
      filePath: "apps/web/src/routes/index.ts",
      packageName: "web",
  ...
  it("spreads confidence 0.4/N across a narrowed ambiguous set with N in [2,3]", () => {
    ...
    const f1 = buildFile({
      filePath: "apps/web/src/a/handler.ts",
      packageName: "web",
      entries: [a],
    });
    const f2 = buildFile({
      filePath: "apps/web/src/b/handler.ts",
      packageName: "web",
      entries: [b],
    });
    const callerFile = buildFile({
  ...
  it("skips entirely — zero edges — when the narrowed ambiguous set exceeds 3 candidates", () => {
    ...
    const files = decls.map((d, i) =>
      buildFile({
        filePath: `apps/web/src/mod${i.toString()}/handler.ts`,
        packageName: "web",
        entries: [d],
      }),
    );
...
describe("resolveCalls — rule 5: method-call receiver-in-scope", () => {
  it("resolves obj.foo() only when the method's class is imported into the calling file", () => {
    ...
    const caller = sym("handler", "FUNCTION", {
      calls: [call("save", "repo")],
    });

    const classFile = buildFile({
      filePath: "packages/db/src/user-repo.ts",
      packageName: "@repo/db",
      entries: [method],
    });
    const decoyFile = buildFile({
      filePath: "packages/db/src/order-repo.ts",
      packageName: "@repo/db",
      entries: [decoyMethod],
    });
    const callerFile = buildFile({
      ...
      entries: [caller],
      imports: [
        namedImport("../../../../packages/db/src/user-repo.js", ["UserRepo"]),
      ],
    });
  ...
  it("does not resolve obj.foo() when neither candidate's class is imported into the calling file (falls to ambiguity, N<=3 spread)", () => {
    ...
    const classFile1 = buildFile({
      filePath: "packages/db/src/user-repo.ts",
      packageName: "@repo/db",
      entries: [method1],
    });
    const classFile2 = buildFile({
      filePath: "packages/db/src/order-repo.ts",
      packageName: "@repo/db",
      entries: [method2],
    });
...
describe("resolveCalls — pathological input bounds", () => {
  it("completes quickly for a call site whose name is 50,000 repeated characters", () => {
    ...
    console.log(
      `resolveCalls with a 50,000-char call name completed in ${elapsedMs.toString()}ms`,
    );
