...
import { disposeAll, MAX_PARSE_CONTENT_BYTES } from "../tree-sitter/parser-pool.js";
import type { ParsedFile } from "../parsed-file.types.js";
import { isParseRefusal, isTrustworthyParse, parseFile, type ParseFileResult } from "./typescript.adapter.js";
...
async function parseOk(language: "typescript" | "tsx" | "javascript", source: string): Promise<ParsedFile> {
  const result: ParseFileResult = await parseFile(`test.${language === "typescript" ? "ts" : language === "tsx" ? "tsx" : "js"}`, language, source);
  if (isParseRefusal(result)) throw new Error(`expected a parsed result, got a refusal: ${result.reason}`);
  return result;
}

function byName<T extends { name: string }>(items: T[], name: string): T {
  const found = items.find((item) => item.name === name);
  if (!found) throw new Error(`expected an entry named "${name}" among: ${items.map((i) => i.name).join(", ")}`);
  return found;
}
...
describe("imports", () => {
  it("default import", async () => {
    ...
    expect(parsed.imports).toContainEqual(expect.objectContaining({ specifier: "./m", default: "Default", isTypeOnly: false, syntax: "static" }));
  });

  it("named import with alias — named[] carries the local bound name, not the original", async () => {
    const parsed = await parseOk("typescript", `import { a, b as c } from "./m";`);
  ...
  it("namespace import", async () => {
    ...
    expect(parsed.imports).toContainEqual(expect.objectContaining({ specifier: "./m", namespace: "ns" }));
  });
  ...
  it("mixed per-specifier type-only import splits into two entries", async () => {
    const parsed = await parseOk("typescript", `import { type T, value } from "./m";`);
  ...
  it("CJS require()", async () => {
    ...
    expect(parsed.imports).toContainEqual(expect.objectContaining({ specifier: "./cjs", syntax: "require" }));
  });

  it("CJS require() with destructuring still produces one import entry for the specifier", async () => {
    const parsed = await parseOk("javascript", `const { a } = require("./cjs");`);
    expect(parsed.imports).toContainEqual(expect.objectContaining({ specifier: "./cjs", syntax: "require" }));
  });

  it("dynamic import() with a literal specifier", async () => {
    const parsed = await parseOk("typescript", `const m = import("./dynamic");`);
    expect(parsed.imports).toContainEqual(expect.objectContaining({ specifier: "./dynamic", syntax: "dynamic" }));
  });

  it("dynamic import() with a non-literal specifier produces no import entry at all — never fabricated", async () => {
    const parsed = await parseOk("typescript", `const p = "./m"; const m = import(p);`);
...
describe("exports", () => {
  it("export { a } from './m' is a re-export — appears in both exports[] and imports[]", async () => {
    ...
    expect(parsed.exports).toContainEqual(expect.objectContaining({ name: "a", reExportFrom: "./m" }));
    expect(parsed.imports).toContainEqual(expect.objectContaining({ specifier: "./m", named: [] }));
  });

  it("export * from './m'", async () => {
    const parsed = await parseOk("typescript", `export * from "./m";`);
    expect(parsed.exports).toContainEqual(expect.objectContaining({ name: "", reExportFrom: "./m" }));
    expect(parsed.imports).toContainEqual(expect.objectContaining({ specifier: "./m" }));
  });

  it("export * as ns from './m'", async () => {
    const parsed = await parseOk("typescript", `export * as ns from "./m";`);
    expect(parsed.exports).toContainEqual(expect.objectContaining({ name: "ns", reExportFrom: "./m" }));
    expect(parsed.imports).toContainEqual(expect.objectContaining({ specifier: "./m" }));
  });

  it("export default function foo() {} is default AND named", async () => {
    const parsed = await parseOk("typescript", `export default function foo() {}`);
    expect(parsed.exports).toContainEqual(expect.objectContaining({ name: "foo", isDefault: true }));
  ...
  it("export default <identifier> uses the real referenced name, not a fabricated one", async () => {
    const parsed = await parseOk("typescript", `const helper = 1; export default helper;`);
    expect(parsed.exports).toContainEqual(expect.objectContaining({ name: "helper", isDefault: true }));
  });

  it("export default () => {} (anonymous) has an empty name, never guessed", async () => {
    const parsed = await parseOk("typescript", `export default () => {};`);
    expect(parsed.exports).toContainEqual(expect.objectContaining({ name: "", isDefault: true }));
  });

  it("export = Foo (TS interop)", async () => {
    const parsed = await parseOk("typescript", `class Foo {} export = Foo;`);
    expect(parsed.exports).toContainEqual(expect.objectContaining({ name: "Foo", isDefault: false }));
  });

  it("indirect export: declared then exported via export { x } elsewhere in the file", async () => {
    const parsed = await parseOk("typescript", `function helper() {} export { helper };`);
...
describe("symbols — kinds", () => {
  ...
  it("abstract class declaration", async () => {
    const parsed = await parseOk("typescript", `abstract class Foo { abstract bar(): void; }`);
    expect(byName(parsed.symbols, "Foo").kind).toBe("CLASS");
  ...
  it("methods — plain, getter, setter, static, constructor, private field methods", async () => {
    ...
    const methodNames = parsed.symbols.filter((s) => s.parentSymbol === "Foo").map((s) => s.name);
    expect(methodNames.sort()).toEqual(["constructor", "create", "plain", "x", "x"].sort());
    for (const name of ["constructor", "plain", "x", "create"]) {
      expect(parsed.symbols.find((s) => s.name === name && s.parentSymbol === "Foo")?.kind).toBe("METHOD");
    }
  });

  it("interface declaration", async () => {
    const parsed = await parseOk("typescript", `interface Foo { bar(): void; }`);
  ...
  it("react component: PascalCase + JSX return, TSX only", async () => {
    const parsed = await parseOk("tsx", `function Greeting() { return <div>hi</div>; }`);
  ...
  it("PascalCase function with no JSX is NOT a react component (requires JSX, not just the name)", async () => {
    const parsed = await parseOk("tsx", `function Registry() { return { get: 1 }; }`);
    expect(byName(parsed.symbols, "Registry").kind).toBe("FUNCTION");
  });

  it("hook: name matches ^use[A-Z]", async () => {
    const parsed = await parseOk("typescript", `function useThing() { return 1; }`);
  ...
  it("a name starting with 'use' but not capitalized after it is not a hook", async () => {
    const parsed = await parseOk("typescript", `function useful() { return 1; }`);
...
describe("symbols — signature and doc comment bounds", () => {
  it("signature excludes the body", async () => {
    const parsed = await parseOk("typescript", `function foo(a: number): number {\n  return a + 1;\n}`);
    const foo = byName(parsed.symbols, "foo");
  ...
  it("a pathologically long generic signature is truncated, not stored whole", async () => {
    const hugeGeneric = Array.from({ length: 200 }, (_, i) => `T${i.toString()} extends object`).join(", ");
...
`;
    const parsed = await parseOk("typescript", source);
    expect(byName(parsed.symbols, "foo").docComment).toBe("Line one.\nLine two.");
  });
});
...
describe("symbols — nesting exclusions", () => {
  ...
  it("an object-literal shorthand method is not emitted as a symbol", async () => {
    const parsed = await parseOk("typescript", `const api = { foo() { return 1; }, bar: () => 2, baz: function () { return 3; } };`);
...
describe("symbols — complexity", () => {
  ...
  it("a branchier function scores strictly higher than a simpler one", async () => {
    const simple = await parseOk("typescript", `function simple() { return 1; }`);
    ...
    expect(byName(branchy.symbols, "branchy").complexity).toBeGreaterThan(byName(simple.symbols, "simple").complexity);
  });

  it("branches inside an inline callback are not counted toward the enclosing function", async () => {
    const withoutCallback = await parseOk("typescript", `function foo(arr: number[]) { return arr; }`);
    ...
    expect(byName(withCallback.symbols, "foo").complexity).toBe(byName(withoutCallback.symbols, "foo").complexity);
  });

  it("a class/interface/enum/type-alias has baseline complexity 1", async () => {
    const parsed = await parseOk("typescript", `class Foo {} interface Bar {} enum Baz { A } type Qux = number;`);
...
describe("calls", () => {
  it("a bare call and a member call are both attributed to the enclosing symbol", async () => {
    ...
    expect(outer.calls).toContainEqual(expect.objectContaining({ name: "foo", receiver: undefined }));
    expect(outer.calls).toContainEqual(expect.objectContaining({ name: "method", receiver: "obj" }));
  });
  ...
  it("a nested-function call is attributed to the innermost tracked enclosing symbol — the untracked inner closure is skipped", async () => {
    ...
    const outer = byName(parsed.symbols, "outer");
    expect(outer.calls.map((c) => c.name).sort()).toEqual(["foo", "inner"].sort());
  });
  ...
  it("a chained call through plain identifiers keeps the whole receiver text", async () => {
    ...
    const foo = byName(parsed.symbols, "foo");
    expect(foo.calls).toContainEqual(expect.objectContaining({ name: "method", receiver: "obj.nested" }));
  });
});

describe("class/interface heritage", () => {
  it("class extends and implements", async () => {
    const parsed = await parseOk("typescript", `class Foo extends Base implements IFoo, IBar {}`);
    const foo = byName(parsed.symbols, "Foo");
  ...
  it("interface extends supports multiple parents", async () => {
    const parsed = await parseOk("typescript", `interface Foo extends Bar, Baz {}`);
    const foo = byName(parsed.symbols, "Foo");
    expect(foo.extends?.sort()).toEqual(["Bar", "Baz"].sort());
  });

  it("an unnameable extends expression (a call) is dropped, never guessed", async () => {
    const parsed = await parseOk("typescript", `class Foo extends mixin(Base) {}`);
    const foo = byName(parsed.symbols, "Foo");
...
describe("parseFile — soft failure and refusal policy", () => {
  it("a well-formed file parses OK with zero parseErrors", async () => {
    const parsed = await parseOk("typescript", `export function foo() { return 1; }`);
  ...
  it("a comments-only file parses OK with no symbols", async () => {
    const parsed = await parseOk("typescript", "// just a comment\n/* and another */\n");
    expect(parsed.parseState).toBe("OK");
    expect(parsed.symbols).toEqual([]);
  });
  ...
  it("an unsupported/unrecognized language is a typed UNSUPPORTED_LANGUAGE refusal, distinguishable from an empty parse", async () => {
    ...
    expect(isParseRefusal(result)).toBe(true);
    if (isParseRefusal(result)) expect(result.reason).toBe("UNSUPPORTED_LANGUAGE");
  });
