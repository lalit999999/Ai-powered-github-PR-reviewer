...
function stubFetch(responses: Response[]) {
  ...
  const fn = vi.fn(async (url: string | URL | Request, init?: RequestInit) => {
    ...
    if (!response) throw new Error(`stubFetch: no scripted response for call #${calls.length.toString()} (${String(url)})`);
    return response;
  });
...
describe("fetchTarballStream — the happy path", () => {
  it("follows the pinned redirect to codeload.github.com and streams the body", async () => {
    ...
    expect(await streamToString(result.stream)).toBe("gzip-bytes-1gzip-bytes-2");

    // Exactly two calls — the metadata-equivalent cost lever this phase's §15 measures.
    expect(calls).toHaveLength(2);
    expect(calls[0]?.url).toBe(`https://api.github.com/repos/${OWNER}/${REPO}/tarball/${SHA}`);
    expect((calls[0]?.init?.headers as Record<string, string>).authorization).toBe(`token ${TOKEN}`);
  ...
  it("never logs the signed codeload URL or the installation token — only the host", async () => {
    const { fn } = stubFetch([redirectResponse(CODELOAD_URL), new Response(new ReadableStream(), { status: 200 })]);
    const logger = noopLogger();

    await fetchTarballStream(INSTALLATION_ID, OWNER, REPO, SHA, { fetchImpl: fn, getToken: async () => TOKEN, logger });

    const allFieldValues = [...logger.info.mock.calls, ...logger.warn.mock.calls, ...logger.debug.mock.calls]
      .flatMap((call) => Object.values((call[1] as Record<string, unknown>) ?? {}))
      .map((value) => JSON.stringify(value));
    expect(allFieldValues.some((value) => value.includes("super-secret-signed-token"))).toBe(false);
    expect(allFieldValues.some((value) => value.includes(TOKEN))).toBe(false);
    expect(allFieldValues.some((value) => value === '"codeload.github.com"')).toBe(true);
  });
});
...
describe("fetchTarballStream — the pinned-host check (§4/§35.9)", () => {
  it("rejects a redirect to a non-codeload.github.com host without following it", async () => {
    const { fn, calls } = stubFetch([redirectResponse("https://evil.example.com/steal-the-token?x=1")]);

    const result = await fetchTarballStream(INSTALLATION_ID, OWNER, REPO, SHA, { fetchImpl: fn, getToken: async () => TOKEN, logger: noopLogger() });
  ...
  it("rejects a redirect to a look-alike host (codeload.github.com.evil.com)", async () => {
    const { fn, calls } = stubFetch([redirectResponse("https://codeload.github.com.evil.com/x")]);

    const result = await fetchTarballStream(INSTALLATION_ID, OWNER, REPO, SHA, { fetchImpl: fn, getToken: async () => TOKEN, logger: noopLogger() });

    expect(result).toEqual({ ok: false, reason: "UNSAFE_REDIRECT", host: "codeload.github.com.evil.com" });
    expect(calls).toHaveLength(1);
  });
  ...
  it("rejects a redirect with no Location header at all", async () => {
    ...
    const result = await fetchTarballStream(INSTALLATION_ID, OWNER, REPO, SHA, { fetchImpl: fn, getToken: async () => TOKEN, logger: noopLogger() });
    expect(result).toEqual({ ok: false, reason: "UNSAFE_REDIRECT", host: "unknown" });
  });
  ...
  it("propagates a throw if codeload.github.com itself tries to redirect again (redirect: 'error' on the second hop)", async () => {
    ...
    await expect(fetchTarballStream(INSTALLATION_ID, OWNER, REPO, SHA, { fetchImpl: fn, getToken: async () => TOKEN, logger: noopLogger() })).rejects.toThrow(
      TypeError,
    );
  });
});
...
describe("fetchTarballStream — error classification (§12)", () => {
  ...
  it("classifies a 500 as a retriable throw — a plain Error, not GithubAccessRevokedError/GithubRateLimitError", async () => {
    ...
    const promise = fetchTarballStream(INSTALLATION_ID, OWNER, REPO, SHA, { fetchImpl: fn, getToken: async () => TOKEN, logger: noopLogger() });
    await expect(promise).rejects.toThrow(/500/);
    await expect(promise.catch((e: unknown) => e)).resolves.not.toBeInstanceOf(GithubAccessRevokedError);
    await expect(promise.catch((e: unknown) => e)).resolves.not.toBeInstanceOf(GithubRateLimitError);
  });
  ...
  it("propagates a network failure (fetch itself rejecting) as a retriable throw", async () => {
    ...
    await expect(
      fetchTarballStream(INSTALLATION_ID, OWNER, REPO, SHA, { fetchImpl: fn, getToken: async () => TOKEN, logger: noopLogger() }),
  ...
  it("throws GithubRateLimitError, carrying the reset time, on a 403 WITH rate-limit headers", async () => {
    const { fn } = stubFetch([new Response(null, { status: 403, headers: { "x-ratelimit-remaining": "0", "retry-after": "90" } })]);
    const error = await fetchTarballStream(INSTALLATION_ID, OWNER, REPO, SHA, { fetchImpl: fn, getToken: async () => TOKEN, logger: noopLogger() }).catch(
      (e: unknown) => e,
    );
    expect(error).toBeInstanceOf(GithubRateLimitError);
    expect((error as GithubRateLimitError).details).toMatchObject({ retryAfterSeconds: 90 });
  });
